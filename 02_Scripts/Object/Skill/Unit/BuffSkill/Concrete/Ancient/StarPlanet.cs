...
namespace ProjectL
{
    [SkillType(SkillAttackType.Buff, SkillGradeType.Acient)]
    public class StarPlanet : BuffSkill
    {
        ...
        protected override void StartBuffSkill(Unit target) 
        {
            ...
            target.UpgradeStatPercentage(StatType.Defense, SkillValue);
            target.UpgradeStatPercentage(StatType.Range, SkillValue);
        }

        protected override void EndBuffSkill(Unit target)
        {
            target.UpgradeStatPercentage(StatType.Damage, SkillValue * -1);
            target.UpgradeStatPercentage(StatType.Defense, SkillValue * -1);
            target.UpgradeStatPercentage(StatType.Range, SkillValue * -1);
        }
    }
}
