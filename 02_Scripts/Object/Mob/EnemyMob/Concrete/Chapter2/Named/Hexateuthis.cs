...
namespace ProjectL
{
    public enum HexateuthisAnimType
    {
        Idle = 1,
        CrawlForward = 5,
        CrawlBackwards = 6,
    ...
    public class Hexateuthis : EnemyMob
    {
        //2분 후 자폭
        ...
        protected override void SpawnAnim()
        {
            ...
            unitAnimator?.SetInteger(MOTION_KEY, (int)HexateuthisAnimType.Idle);
        }
        ...
        protected override void DeathAnim()
        {
            ...
            if (CurrentAnim == (int)HexateuthisAnimType.Death)
            {
                return;
            }

            unitAnimator?.SetInteger(MOTION_KEY, (int)HexateuthisAnimType.Death);
        }
        ...
        protected override void IdleAnim()
        {
            ...
            if (CurrentAnim == (int)HexateuthisAnimType.GetHitFront)
            {
            ...
            if (CurrentAnim == (int)HexateuthisAnimType.Idle)
            {
                return;
            }
        ...
        protected override void AttackAnim()
        {
            ...
            if (CurrentAnim == (int)HexateuthisAnimType.ClawsAttack
                || CurrentAnim == (int)HexateuthisAnimType.GetHitFront)
            {
                if(unitAnimator.GetCurrentAnimatorStateInfo(0).normalizedTime < 1.0f)
                {
                    return;
            ...
            StartAnimationWithReturnIdle(HexateuthisAnimType.ClawsAttack);
        ...
        protected override void StunAnim()
        {
            ...
            StartAnimationWithReturnIdle(HexateuthisAnimType.GetHitFront);
        }
        ...
        protected override void RunAnim(bool isLeft, bool isBack, bool isSide)
        {
            ...
            if (isSide && isLeft)
            {
                unitAnimator?.SetInteger(MOTION_KEY, (int)HexateuthisAnimType.CrawlLeft);
            }
            else if (isSide && !isLeft)
            {
                unitAnimator?.SetInteger(MOTION_KEY, (int)HexateuthisAnimType.CrawlRight);
            }
            else if (isBack)
            {
                unitAnimator?.SetInteger(MOTION_KEY, (int)HexateuthisAnimType.CrawlBackwards);
            }
            else
            {
                unitAnimator?.SetInteger(MOTION_KEY, (int)HexateuthisAnimType.CrawlForward);
            }
        }
        ...
        private void StartAnimationWithReturnIdle(HexateuthisAnimType animType)
        {
