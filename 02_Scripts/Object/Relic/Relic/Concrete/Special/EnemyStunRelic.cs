...
namespace ProjectL
{
    public class EnemyStunRelic : Relic
    {
        ...
        {
            D.SelfEnemyPlayer.onSharedHitMob.Add(Stun);
        }
        ...
        {
            D.SelfEnemyPlayer.onSharedHitMob.Remove(Stun);
        }
        ...
        private void Stun(Mob mob)
        {
            bool isStun = Random.Range(0, 100f) <= stunProbabilityValue;

            if (isStun)
            {
                mob.Stun(stunDurationTime);
            }
        }
    }
}