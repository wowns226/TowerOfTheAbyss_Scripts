...
namespace ProjectL
{
    public class CriticalIncDamageDecRelic : Relic
    {
        public override string SpecialDetailDescription =>
            string.Format(Localization.GetLocalizedString(description)
                , increaseCriProbabilityValue
                , decreaseCriDamageValue);
        
        [SettingValue] 
        private float increaseCriProbabilityValue;
        [SettingValue]
        private float decreaseCriDamageValue;
        
        protected override void InitRelicSet() { }

        protected override void _ActivateCommon()
        {
            Player.AllUpgradeStat(UnitType.Mob, StatType.CriticalProbability, increaseCriProbabilityValue);
            Player.AllUpgradeStat(UnitType.Mob, StatType.CriticalDamagePercentage, decreaseCriDamageValue * -1);
        }
        ...
        {
            Player.AllUpgradeStat(UnitType.Mob, StatType.CriticalProbability, increaseCriProbabilityValue * -1);
            Player.AllUpgradeStat(UnitType.Mob, StatType.CriticalDamagePercentage, decreaseCriDamageValue);
        }
