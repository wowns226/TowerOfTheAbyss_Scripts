...
namespace ProjectL
{
    public class HpIncDefenseDecRelic : Relic
    {
        public override string SpecialDetailDescription =>
            string.Format(Localization.GetLocalizedString(description)
                , increaseHpValue
                , decreaseDefenseValue);
        
        [SettingValue] 
        private float increaseHpValue;
        [SettingValue]
        private float decreaseDefenseValue;
        
        protected override void InitRelicSet() { }

        protected override void _ActivateCommon()
        {
            Player.AllUpgradeStatPercentage(UnitType.Mob, StatType.Hp, increaseHpValue);
            Player.AllUpgradeStatPercentage(UnitType.Mob, StatType.Defense, decreaseDefenseValue * -1);
        }
        ...
        {
            Player.AllUpgradeStatPercentage(UnitType.Mob, StatType.Hp, increaseHpValue * -1);
            Player.AllUpgradeStatPercentage(UnitType.Mob, StatType.Defense, decreaseDefenseValue);
        }
